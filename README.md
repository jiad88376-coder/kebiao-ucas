...
国科大人自己的课表工具：粘贴课程代码，3 秒生成整学期课表；笔记 / 作业 DDL / 考试一站管理，每日消息提醒，云同步互通，可安装成手机 App。还带一个匿名话题漂流玩法「漂流瓶」。一校三地（校本部 · 杭高院 · 海洋学院青岛基地）。
...
- `data/topics.json` 是「漂流瓶」的话题库，**每天 3–5 个话题**，人工手改提交即可（**必须在 sw.js 的 `CORE` 名单里**，否则新话题取不到）；当天话题会同时显示在课表页顶部的「今日主题」栏
- 漂流瓶的账户等级与每日配额（临时 3/3 · 正式 5/5 · 高级 10/10）由服务端 `sql/drift_v2.sql` 权威计算，前端只展示
...
- 数据库：`sql/` 下按序执行 `user_data_setup.sql`、`forum_setup.sql`、`forum_open_read.sql`、`push_setup.sql`、「漂流瓶」用 `drift_setup.sql`
